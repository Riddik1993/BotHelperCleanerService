...
from services.database.database_services import DbService
...
async def main():
    ...
    deleted_lessons_cnt = await db_service.delete_expired_lessons(
        arguments.lessons_ttl_days
    )
    deleted_tasks_cnt = await db_service.delete_expired_tasks(arguments.tasks_ttl_days)
    await sending_service.send_lessons_notification(
        deleted_lessons_cnt, deleted_tasks_cnt
    )
